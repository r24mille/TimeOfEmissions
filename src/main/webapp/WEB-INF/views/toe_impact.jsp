<%@ page contentType="text/html; charset=UTF-8"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
<%@ taglib uri="http://www.springframework.org/tags/form" prefix="form"%>
<%@ taglib prefix="toe" tagdir="/WEB-INF/tags/toe"%>
<!DOCTYPE html>
<html class="no-js">
<toe:head pageTitle="Time-of-Emissions Forecast Impact Simulation"
	description="Simulate the ideal impact of a time-of-emissions signal with automated demand responsef" />
<body>
	<script>
		var margin = {
			top : 20,
			right : 20,
			bottom : 30,
			left : 50
		}, width = 960 - margin.left - margin.right, height = 500 - margin.top
				- margin.bottom;
		
		var x = d3.time.scale().range([ 0, width ]);

		var y = d3.scale.linear().range([ height, 0 ]);

		var xAxis = d3.svg.axis().scale(x).orient("bottom");

		var yAxis = d3.svg.axis().scale(y).orient("left");

		var area = d3.svg.area().x(function(d) {
			return x(d.date);
		}).y0(height).y1(function(d) {
			return y(d.megawatts);
		});

		var svg = d3.select("body").append("svg").attr("width",
				width + margin.left + margin.right).attr("height",
				height + margin.top + margin.bottom).append("g").attr(
				"transform",
				"translate(" + margin.left + "," + margin.top + ")");

		d3.json("/TimeOfEmissions/toe_impact/iso/IESO/date/2014-03-15/json",
				function(error, data) {
					data.oversupply.forEach(function(d) {
						d.date = new Date(d.date);
						d.megawatts = +d.megawatts;
					});

					x.domain(d3.extent(data.oversupply, function(d) {
						return d.date;
					}));
					y.domain([ 0, d3.max(data.oversupply, function(d) {
						return d.megawatts;
					}) ]);

					svg.append("path").datum(data.oversupply).attr("class", "area").attr(
							"d", area);

					svg.append("g").attr("class", "x axis").attr("transform",
							"translate(0," + height + ")").call(xAxis);

					svg.append("g").attr("class", "y axis").call(yAxis).append(
							"text").attr("transform", "rotate(-90)").attr("y",
							6).attr("dy", ".71em").style("text-anchor", "end")
							.text("Oversupply (MW)");
				});
	</script>
</body>
</html>